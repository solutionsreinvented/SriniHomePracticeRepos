...
namespace SlvParkview.FinanceManager.Reporting.Models
{
    ...
    public abstract class ReceiptsReport : Report, IReport
    {
        ...
        [JsonProperty]
        public string TotalReceipt { get => Get<string>(); private protected set => Set(value); }
        ...
        private protected virtual List<ReceiptInfo> ApplyFilterOn(List<ReceiptInfo> payments)
        {
            return _reportOptions.ReceiptModeFilter == ReceiptModeFilter.All
                ? payments
                : payments.Where(p => p.Mode == _reportOptions.ReceiptModeFilter.ToString()).ToList();
        }
