...
namespace SlvParkview.FinanceManager.Reporting.Models.Base
{
    public abstract class Report : PropertyStore, IReport
    {
        ...
        [JsonProperty]
        public string JavaScriptFilePath { get => Get<string>(); private protected set => Set(value); }
        ...
        private protected virtual void CreateHtmlFile()
        {
            ...
            string[] htmlTemplateContents = File.ReadAllLines(Path.Combine(FileServiceProvider.ReportTemplatesDirectory, $"{TemplateFileName}.html"));
            List<string> finalHtmlFileContent = ConcatenateScriptTagIn(htmlTemplateContents, Path.GetFileName(HtmlFilePath));
        ...
        /// </summary>
        private protected virtual void CreateJavaScriptFile()
        {
            JavaScriptFilePath = Path.Combine(_reportTargetDirectory, $"{GetFileName()}.js");

            string jsTemplateFilePath = Path.Combine(FileServiceProvider.ReportScriptsDirectory, $"{TemplateFileName}.js");
            string[] jsTemplateContents = File.ReadAllLines(jsTemplateFilePath);
            string finalJavaScriptFileContent = ConcatenateJsonContentIn(jsTemplateContents);

            File.WriteAllText(JavaScriptFilePath, finalJavaScriptFileContent);
        }
