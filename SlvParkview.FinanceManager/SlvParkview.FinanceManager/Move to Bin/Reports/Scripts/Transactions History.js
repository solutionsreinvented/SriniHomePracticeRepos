...
function populateTransactionDetails() {
  ...
  document.title =
    "Flat Transactions History (" +
    jsonContents.FlatInfo.Description +
    ")(" +
    jsonContents.GeneratedOn +
    ")";
    ...
    for (i = 0; i < transactions.length; i++) {
      summaryTableBody.innerHTML += `
            <tr class="transaction-row">
                ...
                <td>${transactions[i].ReceiptAmount}</td>
                <td>${transactions[i].ReceiptMode}</td>
                <td>${transactions[i].ReceiptCategory}</td>
                <td>${transactions[i].BillAmount}</td>
                <td>${transactions[i].BillCategory}</td>
