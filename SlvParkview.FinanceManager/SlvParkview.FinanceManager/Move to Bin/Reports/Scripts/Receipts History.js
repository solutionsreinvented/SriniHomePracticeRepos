...
var jsonContents = {
  ...
  TotalAmountReceived: "1,61,802.0",
  GeneratedOn: "01 Apr 2023",
  ApartmentName: "SLV Parkview Apartment",
  DocumentTitle: "Receipts History (March 2023 - All)",
};
// #endregion
...
// #region Populate Receipts

function populateReceipts() {
  var receipts = jsonContents.Receipts;
  ...
  // tableTitle.innerHTML = "Receipts Received for " + jsonContents.ReportedMonth + " " + jsonContents.ReportedYear;
    ...
    for (i = 0; i < receipts.length; i++) {
      tableBody.innerHTML += `
            <tr class="transaction-row">
              <td class="paid-on">${receipts[i].ReceivedOn}</td>
              <td class="flat-number">${receipts[i].FlatNumber}</td>
              <td class="owner-name">${receipts[i].OwnerName}</td>
              <td class="amount">${receipts[i].Amount}</td>
              <td class="payment-mode">${receipts[i].Mode}</td>
              <td class="payment-category">${receipts[i].Category}</td>
            </tr>
            `;
    }
  ...
  tableBody.innerHTML += `<tr class="total-collection">
            ...
            <td colspan="1" class="total-collection-amount">${jsonContents.TotalAmountReceived}</td>
        </tr>`;
