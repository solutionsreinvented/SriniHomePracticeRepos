...
namespace ReInvented.Domain.Optimization.Services
{
    public class PlatesOptimizationService
    {
        ...
        public HashSet<PlateGroupDesignResult> OptimizeGroups(IEnumerable<string> groupNames, IEnumerable<ILoadCase> loadCases)
        {
            ...
            {
                PlateGroupDesignResult designResult = OptimizeGroup(gn, loadCases, Criteria.ThreadCount);
        ...
        public PlateGroupDesignResult OptimizeGroup(string groupName, IEnumerable<ILoadCase> loadCases, int nThreads)
        {
            IEnumerable<Plate> plates = Wrapper.Geometry.GetEntitiesInGroup<Plate>(groupName, nThreads);
            double currentThickness = Wrapper.Property.GetPlateThickness(plates.First().Id).A * 1000;

            IEnumerable<PlateCenterResults> results = Wrapper.GetPlateCenterResultsForGroup(groupName, loadCases, nThreads);

            results.ToList().ForEach(r => r.ConvertUsing(UnitsConverter));
            PlateGroupStressSummary governingResult = results.GetGoverningPlateGroupStressSummary(Criteria.LimitingStress, nThreads);
